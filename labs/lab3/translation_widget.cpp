...
void TranslationWidget::SetupUi() {
  ...
  title_label_ = new QLabel(">> DECRYPTION MODULE");
  ...
  timer_label_->setStyleSheet(
      "font-size: 16px; color: #00ffcc; font-weight: 900; font-family: monospace;");
  ...
  progress_bar_->setStyleSheet(
      ...
      "QProgressBar::chunk { background: #00ffcc; border-radius: 0px; }");
  layout->addWidget(progress_bar_);

  // Instruction
  QLabel* instruction = new QLabel("TARGET SEQUENCE:");
  ...
  sentence_label_->setStyleSheet(
      ...
      "background: #09090b; border: 1px solid #00ffcc; border-radius: 0px; padding: 20px; font-family: monospace; "
      "min-height: 60px;");
  layout->addWidget(sentence_label_);

  answer_input_ = new QLineEdit();
  answer_input_->setPlaceholderText("INPUT DECRYPTED SEQUENCE...");
  answer_input_->setStyleSheet(
      "QLineEdit { background: #09090b; color: #e4e4e7; border: 1px solid "
      "#27272a; border-radius: 0px; padding: 14px 16px; font-size: 18px; font-weight: bold; font-family: monospace; }"
      "QLineEdit:focus { border-color: #00ffcc; background: #18181b; }");
  ...
  submit_button_ = new QPushButton("EXECUTE");
