...
void GrammarWidget::SetupUi() {
  ...
  title_label_ = new QLabel(">> SYNTAX MODULE");
  title_label_->setStyleSheet(
      "font-size: 24px; font-weight: 900; color: #e4e4e7; font-family: monospace;");
  ...
  timer_label_->setStyleSheet(
      "font-size: 16px; color: #ff00ff; font-weight: 900; font-family: monospace;");
  status_row->addWidget(timer_label_);

  wrong_count_label_ = new QLabel("❌ 0 / 3");
  wrong_count_label_->setStyleSheet("font-size: 16px; font-weight: bold; color: #ff00ff; font-family: monospace;");
  ...
  progress_bar_->setStyleSheet(
      "QProgressBar { background: #18181b; border: 1px solid #27272a; border-radius: 0px; "
      "color: #e4e4e7; font-size: 12px; font-weight: bold; font-family: monospace; text-align: center; }"
      "QProgressBar::chunk { background: #ff00ff; border-radius: 0px; }");
  layout->addWidget(progress_bar_);

  // Instruction
  QLabel* instruction = new QLabel("SELECT VALID INSTRUCTION:");
  instruction->setStyleSheet("font-size: 14px; font-weight: bold; color: #a1a1aa; margin-top: 10px; font-family: monospace;");
  ...
  sentence_label_->setStyleSheet(
      ...
      "background: #09090b; border: 1px solid #ff00ff; border-radius: 0px; padding: 20px; font-family: monospace; "
      "min-height: 60px;");
  ...
  feedback_label_->setStyleSheet("font-size: 16px; font-weight: bold; min-height: 24px; font-family: monospace;");
  feedback_label_->hide();
  layout->addWidget(feedback_label_);

  submit_button_ = new QPushButton("COMPILE");
  submit_button_->setFixedHeight(52);
  submit_button_->setCursor(Qt::PointingHandCursor);
  submit_button_->setStyleSheet(
      "QPushButton { background: #ff00ff; color: #000000; border: none; "
      "border-radius: 0px; font-size: 18px; font-weight: 900; letter-spacing: 2px; font-family: monospace; }"
      "QPushButton:hover { background: #ff33ff; }"
      "QPushButton:pressed { background: #cc00cc; }"
...
void GrammarWidget::ShowTask() {
  ...
  for (int i = 0; i < task.options.size(); ++i) {
    ...
    radio->setStyleSheet(
        "QRadioButton { color: #a1a1aa; font-size: 16px; font-weight: bold; font-family: monospace; "
        "padding: 12px 16px; background: #09090b; border: 1px solid #27272a; border-radius: 0px; }"
        "QRadioButton:hover { background: #18181b; color: #ff00ff; }"
        "QRadioButton::indicator { width: 18px; height: 18px; }"
        "QRadioButton:checked { border: 1px solid #ff00ff; background: #18181b; color: #ff00ff; }");
